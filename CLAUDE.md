# CLAUDE.md

This file provides guidance to Claude Code (claude.ai/code) when working with code in this repository.

## What this is

「归来」(Guī Lái) — Evan's personal **writing blog + digital garden**. Astro 5 static site, zero client-side JS. Visual direction is **neo-brutalist** (Direction "D2"): ink + warm paper + 马金黄 marigold accent + 朱砂 seal red, hard 3px borders with offset hard shadows, a typographic 归来 masthead with a seal stamp, full-bleed ticker/footer. Display face is 得意黑 Smiley Sans (`cn-fontsource-smiley-sans-oblique-regular`), with Archivo for Latin and JetBrains Mono for data; body is system sans. The home is a wide two-column showcase (hero + feed + sidebar); article/garden detail pages stay a single ~680px reading column. Dark mode follows `prefers-color-scheme` by default but can be overridden by a nav toggle (`<html data-theme="light|dark">`, persisted in `localStorage`). Deployed to GitHub Pages (repo `nefelibata202/nefelibata202.github.io`) and served at the custom domain https://guilai.me/ (the old `nefelibata202.github.io` URL 301-redirects there).

## Commands

```bash
npm install        # install deps (Node >= 20.3)
npm run dev        # local preview at http://localhost:4321
npm run build      # build static output to dist/
npm run preview    # serve the built dist/
npm test           # run the Vitest unit suite
npx vitest run tests/links.test.ts   # run a single test file
```

There is no lint step. CI (`.github/workflows/deploy.yml`) runs `npm ci && npm run build` and deploys `dist/` to Pages on every push to `main`.

## Architecture

Static-only: all HTML is generated at build time. "No client JS" is the default — do not add `<script>` or `client:*` directives without a deliberate reason. Two sanctioned exceptions use small inline `is:inline` scripts (no framework/hydration): (1) the **dark-mode toggle** — a FOUC-guard in `BaseLayout`'s `<head>` applies the saved theme before paint, and `Nav` flips `<html data-theme>` + `localStorage`; (2) **search** — `/search` loads Pagefind's UI from `/pagefind/` (generated by `pagefind --site dist` in the `build` script; `.post` carries `data-pagefind-body` so only article/note bodies are indexed). Pagefind output only exists after a real build, so search is inert under `astro dev`. Bundled (non-`is:inline`) modules are also in play now and stay tiny: `rd-enhance.js` (reveal + parallax + tag chips) on the redesigned list pages, and `article-rail.js` on article pages (~2.5 KB total there; mermaid is a dynamic `import()` so only articles with a diagram pay for it).

**Content is data, not pages.** Three sources drive everything:
- `src/content/articles/*.md` — blog posts. Frontmatter: `title, date, tags[], summary, draft`.
- `src/content/garden/*.md` — digital-garden notes. Frontmatter: `title, updated, tags[]`. Bodies may use `[[wikilink]]`.
- `src/data/favorites.json` — a private media library (电影/剧集/书籍/音乐) with 0–10 scores, genres, year, cover image path, and watched/read status (validated by `src/lib/favorites.ts`). Cover images live in `public/covers/`.

Collections are defined in `src/content.config.ts` (Astro 5 glob loader + zod). **Slug = the collection-relative path of the `.md` file** (= Astro's `entry.id`); slugs are English, titles may be Chinese.

**Wikilinks & backlinks (the signature feature):**
- `src/lib/links.ts` — pure, dependency-free logic: `extractWikiLinks`, `buildLinkMap`, `buildBacklinkIndex`, `toUrl`. This is the unit-tested core; keep it pure.
- `src/lib/wikilink.ts` — a remark plugin that rewrites `[[标题]]` / `[[标题|显示]]` into in-site `<a>` at build time. Matched by exact title; unmatched → plain text.
- `src/lib/linkmap.node.ts` — Node-only (`*.node.ts`): scans `src/content/**/*.md` at config time to build the title→url map the remark plugin needs. Imported by `astro.config.ts`. Skips `draft: true`.
- Backlinks ("反向链接") are computed once in `getStaticPaths` of `src/pages/garden/[slug].astro` and rendered at page bottom via `PostLayout`'s named `<slot name="after" />`.

**Why `linkmap.node.ts` is separate from `links.ts`:** the remark plugin is configured in `astro.config.ts` before Astro loads content, so the link map is built by scanning the filesystem directly. The glob there (`**/*.md`) must stay in sync with `content.config.ts`'s loader pattern, or wikilink targets and page URLs diverge.

**Layout/components:** `src/layouts/BaseLayout.astro` (html shell + Nav + Footer, props `title`/`description?`) → `src/layouts/PostLayout.astro` (garden-note chrome, default slot for body + named `after` slot) or `src/layouts/ArticleLayout.astro` (essay chrome: 篇首 head, 版心 rail, body, `after` slot). Shared bits: `Nav`, `Footer`, `PostList` (`items: {url,title,meta}[]`), `TagChip`, `Backlinks`, `ArticleEnd`. Styling: `src/styles/global.css` (+ `redesign.css` for the `rd-` language, `article.css` for the `ar-` article page).

**Article reading page (`ar-`):** the 篇首 shows tags / title / `summary` as 提要 / date · 字数 · 读时. Left of the body sits the **版心 rail** — a hairline 版口 with a clickable short mark per `h2`, a 朱砂 双鱼尾 that tracks the reading position, and the current section name set vertically. It is built client-side from the DOM by `src/scripts/article-rail.js`, appears only ≥1080px, and degrades to a 2px top progress line on narrow screens. 篇末 is a 归 seal plus one list: **延伸** (the garden notes this essay wikilinks to) when the body has any, otherwise **相关** (up to 3 articles sharing tags).

**Pages:** `index` (home), `articles/` (list + `[slug]`), `garden/` (list + `[slug]`), `tags/[tag]` (aggregates both collections; tag URLs are intentionally non-ASCII), `collections`, `about`, `rss.xml.ts`. Drafts (`draft: true`) are excluded everywhere: home, lists, detail, tags, RSS, and the backlink index.

## Adding content (publish flow)

The site is the publishing end of Evan's Obsidian PKM vault: the `compose` skill writes drafts to the vault's `12-Resource/drafts/`, then Evan **manually copies** the `.md` into `src/content/articles/` or `src/content/garden/`, fixes frontmatter, and pushes. Push to `main` → Actions builds and deploys. Use English filenames (they become the slug).

## Conventions

- Match the existing per-page idiom: small inline `fmt` date helper, server-side `getCollection` in frontmatter, `entry.id` as slug. Don't introduce a framework or client runtime.
- Keep `src/lib/links.ts` and `src/lib/favorites.ts` pure and unit-tested. Page/layout `.astro` files are verified by `npm run build` (+ grep of `dist/`), not unit tests.

## Custom domain

The site is bound to **guilai.me**: `public/CNAME` holds the domain and `astro.config.ts` `site` = `https://guilai.me` (root path, no `base` — user-site repo). DNS points at GitHub Pages, and the old `nefelibata202.github.io` URL 301-redirects to guilai.me. To change the domain: update `public/CNAME` + `site` together and repoint DNS.

## Project docs

The design spec and the task-by-task implementation plan are committed under `docs/superpowers/`.
