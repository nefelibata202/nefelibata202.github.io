...
"归图"是一张可交互的航海图风格页面，作为个人导览入口。页面上有 7 座风格化岛屿，一艘小船停在中央"母港"。用户点击任意岛屿，船只沿贝塞尔曲线驶向目标，抵达后右侧面板（移动端为底部抽屉）展开显示该岛的内容。

---

## 内容

7 座岛屿，分两类：**个人岛**（左侧）和**信息岛**（右侧）。

### 个人岛（内容已确定）

| 岛屿 ID | 显示名 | 内容摘要 |
|---------|--------|---------|
| `people` | 喜欢的人物 | 演员：艾德里安·布罗迪、罗伯特·德尼罗、菅田将晖；作家：刘震云、东野圭吾、马伯庸；创作者：梁文道、孟岩 |
| `interests` | 个人兴趣 | 历史故事、悬疑犯罪、知识管理、精酿啤酒、英语学习、运动锻炼 |
| `philosophy` | 人生哲学 | ①接受自己是普通人 ②以我为主，为我所用 ③实践出真知 ④动态系统理论&优化心态 ⑤需求导向 |
| `travel` | 旅行计划 | 福冈（深度放松之旅）、Fuji 音乐节（音乐之旅）、新疆旅游 |

### 信息岛（内容待填充，先占位）

| 岛屿 ID | 显示名 | 说明 |
|---------|--------|------|
| `info-sources` | 高质量的信息源 | 值得长期订阅的媒体、播客、Newsletter、作者 |
| `fragmented-time` | 碎片时间 | 利用碎片化时间可以做的事清单 |
| `knowledge-gaps` | 知识库缺口 | 想补但还没补的知识领域 |

内容以 Markdown 存储于 `src/content/guide/`（新集合），每个文件对应一座岛屿。信息岛的 3 个文件初始为占位内容，后续编辑填充。
...
| `--island-gold` | `rgba(242,179,18,0.18)` | 旅行计划、喜欢的人物的岛屿叠色 |
...
### SVG 地图布局（参考坐标，viewBox 900×520）

7 座岛屿分两个聚落，母港居中——**左聚落：个人岛**，**右聚落：信息岛**：

```
(0,0)────────────────────────────────────────(900,0)
  │                                               │
  │  人物(110,90)   人生哲学(290,80)              │
  │                                               │
  │  个人兴趣(100,230)  旅行(280,220)  [母港⛵](470,270)
  │                                               │
  │                          信息源(640,130)      │
  │                                    知识缺口(790,250)
  │                          碎片时间(660,390)    │
...
- 左下角：图例铭牌（归图 · 个人导览 · 2026）
- 右下角：罗盘玫瑰（金色，纯装饰）
- 两个聚落之间的海峡区域放置母港，视觉上有"连接两岸"的感觉
...
```
src/content/guide/
  people.md          ← 喜欢的人物（内容已确定）
  interests.md       ← 个人兴趣（内容已确定）
  philosophy.md      ← 人生哲学（内容已确定）
  travel.md          ← 旅行计划（内容已确定）
  info-sources.md    ← 高质量的信息源（占位）
  fragmented-time.md ← 碎片时间（占位）
  knowledge-gaps.md  ← 知识库缺口（占位）
...
构建时：
  getCollection('guide')
    → 7 个 <div data-island="travel"> 隐藏面板
    → 内容已完全渲染为 HTML
